...
<div class="max-w-md mx-auto my-8 md:my-16">
    <div class="bg-white rounded-3xl p-8 md:p-12 border border-[#e4e4e7] shadow-sm relative z-10">
        ...
        <div class="text-center mb-8">
            <h2 class="text-3xl font-extrabold text-[#18181b] tracking-tight">Welcome Back</h2>
            <p class="text-xs text-black/50 font-semibold mt-2">Sign in to access your AgroGeo dashboard</p>
        </div>
        ...
        <form action="{{ route('login') }}" method="POST" class="flex flex-col gap-5">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="email" id="email" name="email" class="w-full bg-[#f8f8f6] border border-[#e4e4e7] rounded-full px-5 py-3 text-sm focus:outline-none focus:border-[#18181b] transition font-medium" value="{{ old('email') }}" required autofocus>
            </div>
            
            <!-- Password -->
            <div class="flex flex-col gap-2">
                <label for="password" class="text-[10px] font-bold text-black/60 uppercase tracking-wider">Password</label>
                <input type="password" id="password" name="password" class="w-full bg-[#f8f8f6] border border-[#e4e4e7] rounded-full px-5 py-3 text-sm focus:outline-none focus:border-[#18181b] transition font-medium" required>
            </div>
        ...
        <div class="mt-8 text-center pt-6 border-t border-[#e4e4e7]/60">
            <p class="text-xs text-black/50 font-medium">
                ...
                <a href="{{ route('register') }}" class="text-[#18181b] font-bold hover:underline transition">Register here</a>
            </p>
        </div>
    </div>
</div>
